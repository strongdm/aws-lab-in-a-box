...
#
# This gate blocks the Windows target and the ADCS/NDES server
# until StrongDM reports the domain controller as reachable. Its
# health check is a genuine readiness signal because the DC
# script only re-enables NLA at the very end of the sequence.
#
# Requires the sdm CLI and jq on PATH. Without them the wait
# fails fast with a clear message, and the two-apply route still
# works: deploy the DC first, then the Windows target or ADCS.
#--------------------------------------------------------------

resource "terraform_data" "dc_ready" {
  count = (var.create_domain_controller && (var.create_windows_target || var.create_adcs)) ? 1 : 0
